...
const Poll = ({ poll, auth, vote }) => {
  ...
  if (auth.isAuthenticated && auth.user.username === "admin") {
    ...
    if (!auth.isAuthenticated) {
      return <div className='auth_vote'>Please login to vote or to see the available options!</div>;
    }
    if (found) {
      return <div className='found'>Thanks for your vote! You've already voted! </div>;
    }
    return (
      <div>
        <div className='found'>
